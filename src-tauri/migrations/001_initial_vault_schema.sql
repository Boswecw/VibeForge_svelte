-- VibeForge Local Evidence Vault — initial schema
-- Ported verbatim from docs/plans/04_DATA_MODEL_SQL.md (VibeForge Plan Set v2 — Premium).
-- Do not hand-edit after this migration has been applied anywhere — sqlx checksums
-- migration files and will hard-error on mismatch. Schema changes are new migrations.
...
-- Additional indexes not in the source doc's index list but needed for doctor's anti-join checks
CREATE INDEX idx_patch_receipt_patch ON patch_receipt(patch_id);
CREATE INDEX idx_patch_receipt_previous_hash ON patch_receipt(previous_receipt_hash);
